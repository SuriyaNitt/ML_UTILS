...
def calc_opticalflow(video, numFrames, typeOfFlow = 2, write_optflow = 0):
    #X = loadDataFromCaches([cacheNum], 224, 224)
    X = np.array(video, dtype='uint8')
    print X.shape
    opticalFlowShape = (X.shape[0] - 1, X.shape[1], X.shape[2], X.shape[3])
    opticalFlow = np.ndarray(opticalFlowShape, dtype='uint8')
...
# Mask image for drawing purpose
    ...
    if typeOfFlow == '1':
        ...
        while(frameNum<numFrames):
            ...
            opticalFlow[frameNum-1] = drawFlow
